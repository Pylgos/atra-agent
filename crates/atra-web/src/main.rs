use std::{
    env, fs,
    os::unix::{fs::{OpenOptionsExt, PermissionsExt}, process::CommandExt},
    path::PathBuf,
...
use tokio::{net::TcpListener, time::{Instant, sleep}};
...
async fn run(args: Args) -> Result<()> {
    match args.command {
        ...
        Some(WebCommand::Status) => status(args.port.unwrap_or(DEFAULT_PORT)).await,
        Some(WebCommand::Stop) => stop(args.port),
...
async fn serve(port: u16) -> Result<()> {
    ...
    let listener = TcpListener::bind(("127.0.0.1", port))
        .await
        .with_context(|| format!("cannot bind http://127.0.0.1:{port}; the port is already in use"))?;
    write_daemon(&runtime, &Daemon { pid: std::process::id(), port })?;
...
async fn open(requested_port: Option<u16>) -> Result<()> {
    let port = match requested_port {
        Some(port) => port,
        None => running_daemon().map_or(DEFAULT_PORT, |daemon| daemon.port),
    };
...
fn stop(requested_port: Option<u16>) -> Result<()> {
    let runtime = runtime_dir()?;
    let path = runtime.join("daemon.json");
    let daemon: Daemon = serde_json::from_slice(
        &fs::read(&path).with_context(|| "Web daemon is not running")?
    ).context("failed to decode Web daemon state")?;
    if requested_port.is_some_and(|port| daemon.port != port) {
        bail!("running Web daemon uses port {}, not {}", daemon.port, requested_port.unwrap());
    }
...
fn spawn(port: u16) -> Result<()> {
    ...
    command.args(["--port", &port.to_string(), "serve"])
        .stdin(Stdio::null()).stdout(log).stderr(stderr);
    unsafe { command.pre_exec(|| rustix::process::setsid().map(|_| ()).map_err(std::io::Error::from)); }
...
async fn healthy(port: u16) -> bool {
    ...
    response.json::<serde_json::Value>().await.ok()
        .and_then(|value| value.get("service").and_then(|value| value.as_str()).map(str::to_owned))
        .as_deref() == Some("atra-web")
}

fn runtime_dir() -> Result<PathBuf> {
    let path = match xdg::BaseDirectories::new().get_runtime_directory() {
        Ok(path) => path.join("atra-web"),
        Err(_) => PathBuf::from(format!("/tmp/atra-web-{}", rustix::process::getuid().as_raw())),
    };
    ensure_private(&path)?;
    Ok(path)
}
...
fn controller_runtime() -> PathBuf {
    env::var_os("ATRA_RUNTIME_DIR").map(PathBuf::from).unwrap_or_else(|| {
        match xdg::BaseDirectories::new().get_runtime_directory() {
            Ok(path) => path.join("atra"),
            Err(_) => PathBuf::from(format!("/tmp/atra-{}", rustix::process::getuid().as_raw())),
        }
    })
}
...
fn ensure_private(path: &std::path::Path) -> Result<()> {
    ...
    if !metadata.is_dir() || metadata.file_type().is_symlink()
        ...
        bail!("{} must be a private directory with mode 0700", path.display());
    }
    Ok(())
}
...
fn write_daemon(runtime: &std::path::Path, daemon: &Daemon) -> Result<()> {
    let path = runtime.join("daemon.json");
    let temporary = runtime.join(format!(".daemon.{}.tmp", std::process::id()));
    let result = (|| {
        fs::write(&temporary, serde_json::to_vec(daemon)?)?;
